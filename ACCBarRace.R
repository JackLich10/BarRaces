...
anim <- result_formatted %>%
  ...
  mutate(game_number = factor(game_number)) %>% 
  ggplot(aes(y = rank, group = shooter)) +
  geom_tile(aes(x = cumulative_pts/2, height = 0.8, width = cumulative_pts,
                color = team, fill = team), 
            alpha = 0.8, size = 1.2) +
  geom_text(aes(x = cumulative_pts, label = paste(shooter, " ")), 
            hjust = 1.15, color = "white", fontface = "bold", size = 6.5) +
  geom_text(aes(x = cumulative_pts, label = format(cumulative_pts, nsmall = 0)),
            hjust = -1.75, color = "black", fontface = "bold", size = 5.5) +
  geom_image(aes(x = cumulative_pts, y = rank, image = logo_url), 
             asp = 1.5, size = 0.03, inherit.aes = F) +
  scale_x_continuous(expand = expansion(mult = c(0, 0.1)), breaks = seq(0, 400, by = 50)) +
  scale_y_reverse() +
  scale_fill_manual(values = fill) +
  scale_color_manual(values = color) +
  transition_states(game_number, transition_length = 1, state_length = 0, wrap = FALSE) +
  ...
  theme(legend.position = "none",
        ...
        plot.subtitle = element_text(size = 28, face = "italic", hjust = 0),
        plot.caption = element_text(size = 18, face = "italic")) +
  labs(title = "ACC Points Leaders During Conference Play",
       subtitle = "Through {closest_state} Conference Game(s)",
       x = "Cumulative Points Scored",
       y = NULL,
       caption = "2019-20 Season | data via @ncaahoopR")
...
for_mp4 <- animate(anim, nframes = 1800, fps = 40, width = 1600, height = 900,
                   start_pause = 10, end_pause = 10,
                   renderer = ffmpeg_renderer())

for_mp4 <- animate(anim, nframes = 40, fps = 10, width = 1600, height = 900,
                   start_pause = 10, end_pause = 10,
                   renderer = ffmpeg_renderer())

for_mp4

anim_save(filename = "ACCPointsLeaders.mp4", 
          animation = animate(anim, nframes = 1800, fps = 40, height = 900, width = 1600, 
                              renderer = ffmpeg_renderer()))
